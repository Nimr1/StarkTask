...
var cephes__namespace = /*#__PURE__*/_interopNamespace(cephes);
...
var rotation_matrix = function rotation_matrix(angle, axis) {
  ...
  var R = math__namespace.zeros([3, 3]);
  R[axis][axis] = 1;
  R[a1][a1] = c;
  R[a1][a2] = -s;
  R[a2][a1] = s;
...
var coe2rv = function coe2rv(mu, p, ecc, inc, raan, argp, nu) {
  var pqw = math__namespace.dotMultiply([[Math.cos(nu), Math.sin(nu), 0], [-Math.sin(nu), ecc + Math.cos(nu), 0]], [[0, 0, 0].fill(p / (1 + ecc * Math.cos(nu)), 0, 3), [0, 0, 0].fill(Math.sqrt(mu / p), 0, 3)]);
  var rm = rotation_matrix(raan, 2);
  rm = math__namespace.multiply(rm, rotation_matrix(inc, 0));
  rm = math__namespace.multiply(rm, rotation_matrix(argp, 2));
  return math__namespace.multiply(pqw, math__namespace.transpose(rm));
};
var rv2coe = function rv2coe(mu, r, v) {
  var tol = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 1e-8;
  var raan, argp, nu;
  var h = math__namespace.cross(r, v);
  var n = math__namespace.cross([0, 0, 1], h);
  var e = math__namespace.divide(math__namespace.subtract(math__namespace.multiply(math__namespace.dot(v, v) - mu / math__namespace.norm(r), r), math__namespace.multiply(math__namespace.dot(r, v), v)), mu);
  var ecc = math__namespace.norm(e);
  var p = math__namespace.dot(h, h) / mu;
  var inc = Math.acos(h[2] / math__namespace.norm(h));
  var circular = ecc < tol;
  var equatorial = Math.abs(inc) < tol;
  if (equatorial && !circular) {
    raan = 0;
    argp = Math.atan2(e[1], e[0]) % (2 * Math.PI);
    nu = Math.atan2(math__namespace.dot(h, math__namespace.cross(e, r)) / math__namespace.norm(h), math__namespace.dot(r, e));
  } else if (!equatorial && circular) {
    raan = modulo(Math.atan2(n[1], n[0]), 2 * Math.PI);
    argp = 0;
    nu = Math.atan2(math__namespace.dot(r, math__namespace.cross(h, n)) / math__namespace.norm(h), math__namespace.dot(r, n));
  } else if (equatorial && circular) {
    raan = 0;
    argp = 0;
    ...
    var mu_a = mu * a;
    if (a > 0) {
      var e_se = math__namespace.dot(r, v) / Math.sqrt(mu_a);
      var e_ce = math__namespace.norm(r) * math__namespace.dot(v, v) / mu - 1;
      var E = Math.atan2(e_se, e_ce);
      nu = E_to_nu(E, ecc);
    } else {
      var e_sh = math__namespace.dot(r, v) / Math.sqrt(-mu_a);
      var e_ch = math__namespace.norm(r) * Math.pow(math__namespace.norm(v), 2) / mu - 1;
      var F = Math.log((e_ch + e_sh) / (e_ch - e_sh)) / 2;
      nu = F_to_nu(F, ecc);
    }
    raan = modulo(Math.atan2(n[1], n[0]), 2 * Math.PI);
    var px = math__namespace.dot(r, n);
    var py = math__namespace.dot(r, math__namespace.cross(h, n)) / math__namespace.norm(h);
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          ...
          _context.next = 8;
          return cephes__namespace.compiled;
        case 8:
          validateGravitationalParam(mu);
          validatePositions(r1, r2);
          c = math__namespace.subtract(r2, r1);
          c_norm = math__namespace.norm(c);
          r1_norm = math__namespace.norm(r1);
          r2_norm = math__namespace.norm(r2);
          s = math__namespace.multiply(math__namespace.add(r1_norm, r2_norm, c_norm), 0.5);
          i_r1 = math__namespace.divide(r1, r1_norm);
          i_r2 = math__namespace.divide(r2, r2_norm);
          i_h = math__namespace.cross(i_r1, i_r2);
          i_h = math__namespace.divide(i_h, math__namespace.norm(i_h));
          ll = Math.sqrt(1 - Math.min(1.0, c_norm / s));
          if (i_h[2] < 0) {
            ll = -ll;
            i_t1 = math__namespace.cross(i_r1, i_h);
            i_t2 = math__namespace.cross(i_r2, i_h);
          } else {
            i_t1 = math__namespace.cross(i_h, i_r1);
            i_t2 = math__namespace.cross(i_h, i_r2);
          }
          if (!prograde) {
            ll = -ll;
            i_t1 = math__namespace.multiply(-1, i_t1);
            i_t2 = math__namespace.multiply(-1, i_t2);
          }
          ...
          v1 = math__namespace.add(math__namespace.multiply(V_r1, math__namespace.divide(r1, r1_norm)), math__namespace.multiply(V_t1, i_t1));
          v2 = math__namespace.add(math__namespace.multiply(V_r2, math__namespace.divide(r2, r2_norm)), math__namespace.multiply(V_t2, i_t2));
...
var _tofEquationY = function _tofEquationY(x, y, T0, ll, M) {
  ...
  if (M == 0 && Math.sqrt(0.6) < x && x < Math.sqrt(1.4)) {
    ...
    var Q = 4 / 3 * cephes__namespace.hyp2f1(3, 1, 5 / 2, S_1);
    T_ = (Math.pow(eta, 3) * Q + 4 * ll * eta) * 0.5;
  } else {
    var psi = _computePsi(x, y, ll);
    T_ = math__namespace.divide(math__namespace.divide(psi + M * Math.PI, Math.sqrt(Math.abs(1 - Math.pow(x, 2)))) - x + ll * y, 1 - Math.pow(x, 2));
  }
  return T_ - T0;
};
    ...
    return _regeneratorRuntime().wrap(function _callee2$(_context2) {
      while (1) switch (_context2.prev = _context2.next) {
        ...
        case 16:
          ...
          deltaV = math__namespace.norm(math__namespace.subtract(vi1, vf1)) + math__namespace.norm(math__namespace.subtract(vi2, vf2));
